...
┌───────▼────────┐      ┌─────────▼──────────┐
│   Solana       │      │      NEAR          │
│  - ZK Launchpad│      │  - NEAR Intents    │
│    Program     │      │  - OneClick Swap   │
│  - SPL Tokens  │      │  - Smart Contracts │
└────────────────┘      └────────────────────┘
        │                         │
        └────────────┬────────────┘
                     │
...
Built with Next.js 15 and TypeScript, the frontend provides:

- **Token Creation Interface**: User-friendly forms for launching tokens
- **Launch Management**: Create and manage token launches
- **Wallet Integration**: Support for Solana and NEAR wallets
- **Token Metadata Management**: Handle token metadata directly in frontend
- **IPFS Storage**: Client-side IPFS integration for immutable metadata
- **Proof Handling**: Download, store, and manage ZK proof ZIP files
- **Query Interface**: Query launches and active token sales
- **Claim Interface**: Claim tokens using ZK proofs
...
**Program ID (Devnet):** `HDFv1zjKQzvHuNJeH7D6A8DFKAxwJKw8X47qW4MYxYpA`
