...
app.post(
  '/whitelist',
  zValidator('json', z.object({ tokenAddress: z.string().min(1) })),
  async (c) => {
    try {
      const requestData = c.req.valid('json');
      const result = await meteoraService.whiteListToken(requestData.tokenAddress);
      return c.json(
        {
          success: true,
          data: result,
...
app.post(
  ...
  async (c) => {
    try {
      const requestData = c.req.valid('json');
      const result = await meteoraService.unWhitelistToken(requestData.tokenAddress);
      return c.json(
        {
          success: true,
          data: result,
